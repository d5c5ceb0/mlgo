...
func (tensor *TensorOnGraph) Encoding(toBigEndian bool) []byte {
	...
	for i := 0; i < len(tensor.Data); i++ {
		data = append(data, common.Float32ToBytes(tensor.Data[i].Float32(), toBigEndian)...)
	}
...
func DecodeTensorOnGraph(data []byte, fromBigEndian bool, currentBigEndian bool) *TensorOnGraph {
	...
	t += 4
	tensorData := make([]float16.Float16, 0)
	if currentBigEndian && fromBigEndian {
		...
		tensorData = make([]float16.Float16, dataSize)
		for i := 0; i < int(dataSize); i++ {
			tensorData[i] = float16.Fromfloat32(common.BytesToFloat32(data[t:t+4], fromBigEndian))
			t += 4
		}
	}
