...
func NewTensor1DWithData(ctx *Context, dt DType, ne0 uint32, data []float8.Float8) *Tensor {
...
func NewTensor2DWithData(ctx *Context, dt DType, ne0, ne1 uint32, data []float8.Float8) *Tensor {
...
func NewTensor(ctx *Context, dt DType, dims uint32, ne0, ne1, ne2, ne3 uint32, data []float8.Float8) *Tensor {
	...
	if data == nil {
		result.Data = make([]float8.Float8, total, total) // &newData
	} else {
		result.Data = data
	}
...
func Rope(ctx *Context, a *Tensor, past, dims, mode uint32) *Tensor {
	...
	b.Data[0] = float8.ToFloat8(float32(past))
	b.Data[1] = float8.ToFloat8(float32(dims))
	b.Data[2] = float8.ToFloat8(float32(mode))
...
func SetFP32(tensor *Tensor, value float32) *Tensor {
	...
	for i := uint32(0); i < n; i++ {
		...
		tensor.Data[i] = float8.ToFloat8(value)
	}
	return tensor
}
...
func VecCopyFP32(n uint32, y, x []float8.Float8) {
	for i := uint32(0); i < n; i++ {
		y[i] = x[i]
	}
}
		...
		for i02 := uint32(0); i02 < ne02; i02++ {
			for i01 := uint32(ith); i01 < ne01; i01 += nth {
				...
				for i := uint32(0); i < ne00; i++ {
					y[i] = float8.ToFloat8(float8.ToFloat32(x[i]) * scale)
				}
			}
		}
	}
...
// ggml_vec_scale_f32
func VecScaleFP32(n uint32, y []float8.Float8, v float8.Float8) {
	for i := uint32(0); i < n; i++ {
		y[i] *= v
	}
}
...
func VecReluFP32(n uint32, y, x []float8.Float8) {
	for i := uint32(0); i < n; i++ {
		if x[i] > 0 {
			y[i] = x[i]
		} else {
			y[i] = float8.ToFloat8(0)
		}
	}
}
...
func VecMulFP32(n uint32, z, x, y []float8.Float8) {
	for i := uint32(0); i < n; i++ {
		z[i] = x[i] * y[i]
	}
}
	...
	*/
	...
	for ir := uint32(ir0); ir < ir1; ir++ {
		...
		for ic := uint32(0); ic < ne11; ic++ {
			...
			sum := float32(0.0)
			for i := uint32(0); i < ne00; i++ {
				sum += float8.ToFloat32(src0Ptr[i] * src1Ptr[i])
			}

			dst.Data[i0*nb0+ic*nb1+i2*nb2+i3*nb3] = float8.ToFloat8(sum)
		}
	}
		...
		for i2 := modeCount; i2 < ne2; i2++ {
			...
			for i1 := uint32(0); i1 < ne1; i1++ {
				for i0 := 0; i0 < int(dims); i0 += 2 {
					...
					dstData[0] = float8.ToFloat8(float32(x0*cosTheta - x1*sinTheta))
					dstData[1] = float8.ToFloat8(float32(x0*sinTheta + x1*cosTheta))
				}
			}
		}
	}
		...
		for j := uint32(0); j < nr; j++ {
			for i := pastCount; i < nc; i++ {
				if i > pastCount+j {
					...
					dst.Data[k*dst.NB[2]/4+j*dst.NB[1]/4+i*dst.NB[0]/4] = float8.ToFloat8(float32(math.Inf(-1))) // TODO Use const
				}
			}
		}
	}
...
func VecMaxFP32(n uint32, x []float8.Float8) float32 {
	max := float32(math.Inf(-1)) // TODO use constant
	for i := uint32(0); i < n; i++ {
		max = maxFloat(max, float8.ToFloat32(x[i]))
	}
	return max
}
...
func ComputeForwardSoftMaxFP32(params *ComputeParams, src0, dst *Tensor) {
	...
	for i1 := ir0; int(i1) < ir1; i1++ {
		...
		//var bits uint16
		for i := 0; i < int(nc); i++ {
			if p[i] == float8.ToFloat8(negInf) { // TODO use constant
				p[i] = 0.0
			} else {
				...
				val := float32(math.Exp(float64(float8.ToFloat32(p[i]) - max)))
				sum += val
				p[i] = float8.ToFloat8(val)
			}
		}

		////assert(sum > 0.0f);
		sum = 1.0 / sum
		VecScaleFP32(nc, p, float8.ToFloat8(sum))
	}
...
func VecAddFP32(n uint32, z, x, y []float8.Float8) {
	for i := uint32(0); i < n; i++ {
		z[i] = x[i] + y[i]
	}
}
...
func VecSiluFP32(n uint32, y, x []float8.Float8) {
	for i := uint32(0); i < n; i++ {
		y[i] = float8.ToFloat8(SiluFP32(float8.ToFloat32(x[i]))) // ggml_silu_f32
	}
}
